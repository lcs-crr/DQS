...
class AnomalyDetector(base_class.BaseProcessor):
    ...
    def _find_detection_delay(
        ...
        assert detection_score.ndim == 1, (
            "detection_score must be a 1D numpy array of shape (time_steps,)!"
        )
        ...
        delay = abs(
            (time_step_detection + rev_window_penalty - anomaly_start)
            / self.sampling_rate
        )
    ...
    def extract_groundtruth(
        ...
        groundtruth_labels = [
            ...
            for idx_data, data_ts in enumerate(input_list)
        ]
    ...
    def evaluate_online(
                ...
                if np.sum(detection_score >= threshold) > 0:
                    ...
                    if self.calculate_delay:
                        if self.reverse_window_penalty:
                            delay, _ = self._find_detection_delay(
                                detection_score,
                                threshold,
                                len(detection_score),
                                groundtruth_start,
        ...
        groundtruth_labels, predicted_labels = self._correct_labels(
