...
class Inferencer(base_class.BaseProcessor):
    ...
    def _first_reverse_window_array(
        ...
        indices = (
            np.arange(num_windows - 1) * self.window_shift
        )  # Compute start indices for each window
        data[indices[:, None] + np.arange(self.window_shift), :] = input_windows[
            :-1, : self.window_shift
        ]
    ...
    def inference_list(
        ...
        for data_ts in tqdm(input_list):
            file_name = data_ts.dtype.metadata["file_name"]
            detection_score, output = _inference_array[model_name](model, data_ts)
            detection_score = detection_score.astype(
