...
for seed, fold_idx in itertools.product(seeds, folds):
    ...
    for split_idx, split in enumerate(splits):
        ...
        for _ in range(BUDGET):
            selection_idx = np.argmin(abs([np.max(candidate_score) for candidate_score in candidate_score_list] - threshold))
...
try:
    with pd.ExcelWriter(os.path.join(model_path, 'results.xlsx'), mode='a', if_sheet_exists='overlay') as writer:
        results.to_excel(
            writer,
            index=False,
            sheet_name=f'unc_{BUDGET}_{int(MISLABEL_PROB * 100)}'
        )
finally:
