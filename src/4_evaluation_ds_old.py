    ...
    for split_idx, split in enumerate(splits):
        ...
        for _ in range(BUDGET):
            if len(query_score_list) == 0:
                selection_idx = np.random.randint(len(candidate_score_list))
            else:
                distance_matrix = np.zeros((len(query_score_list), len(candidate_score_list)))
                for idx_query, detection_score_ts in enumerate(query_score_list):
                    for idx_candidate, candidate_score_ts in enumerate(candidate_score_list):
                        distance_matrix[idx_query, idx_candidate] = dtw.distance_fast(detection_score_ts.astype(np.dtype(np.double)), candidate_score_ts.astype(np.dtype(np.double)))
                arg_min = np.unravel_index(np.argmin(distance_matrix), distance_matrix.shape)
                sim_candidate = candidate_score_list[arg_min[1]]
                distance_matrix = np.zeros(len(candidate_score_list))
                for idx_candidate, candidate_score_ts in enumerate(candidate_score_list):
                    distance_matrix[idx_candidate] = dtw.distance_fast(sim_candidate.astype(np.dtype(np.double)), candidate_score_ts.astype(np.dtype(np.double)))
                selection_idx = np.argmax(distance_matrix)
...
if not os.path.isfile(os.path.join(model_path, 'results_ds_old.xlsx')):
    # Create and save a valid Excel file
    wb = openpyxl.Workbook()
    wb.save(os.path.join(model_path, 'results_ds_old.xlsx'))

# Use a try-finally block to ensure proper handling
try:
    with pd.ExcelWriter(os.path.join(model_path, 'results_ds_old.xlsx'), mode='a', if_sheet_exists='overlay') as writer:
        results.to_excel(
            writer,
            index=False,
    ...
    try:
        workbook = openpyxl.load_workbook(os.path.join(model_path, 'results_ds_old.xlsx'))
        if 'Sheet' in workbook.sheetnames:
            del workbook['Sheet']
        workbook.save(os.path.join(model_path, 'results_ds_old.xlsx'))
