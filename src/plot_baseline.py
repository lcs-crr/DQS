...
df_upper_baseline = pandas.read_excel(
    os.path.join(model_path, "results_best.xlsx"),
...
df_lower_baseline = pandas.read_excel(
    os.path.join(model_path, "results_baseline.xlsx"),
    # sheet_name=combination,
    header=0,
    usecols=[0, 1, 2, 3, 4, 5, 6, 7],
)
df_upper_baseline_mean = (
    df_upper_baseline.groupby("Split")
    .mean()
    .reset_index()
    .drop(columns=["Split", "Fold", "Seed"])
)
df_lower_baseline_mean = (
    df_lower_baseline.groupby("Split")
    .mean()
    .reset_index()
    .drop(columns=["Split", "Fold", "Seed"])
)
